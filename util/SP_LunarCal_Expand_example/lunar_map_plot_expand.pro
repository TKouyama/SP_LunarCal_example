...
function map_read_photometric_params, ifname,pix_n,wav,b0,db0,h,dh,c,dc,g,dg,r_mean
  ...
  while(not EOF(1)) do begin
    ...
    readf,1,tmp_pix_n,tmp_wav,tmp_B0,tmp_h,tmp_c,tmp_g
    pix_n[count] = tmp_pix_n
    wav[count] = tmp_wav
    b0[count] = tmp_b0
    h[count] = tmp_h
...
pro lunar_map_plot,obs_geo,ccd_ref $
  ...
  ;ifname_albedo_g_h = fldname+'High_albedo_sel_expand.txt'
  ;ifname_albedo_g_m = fldname+'Mid_albedo_sel_expand.txt'
  ;ifname_albedo_g_l = fldname+'Low_albedo_sel_expand.txt'

  ;; SP data 0.5x0.5 with 204 bands
  ifname_ref = fldname +'avgradf_spl2cv2_720_try6and7.img'
  n_bands = 204l
  ...
  ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ;; 0.5x0.5 deg Refrectance and Albedo group (data_type 5 = 64bit)
  ;map_sp_ref = read_binary(ifname_ref, data_type=5, data_dims=[360*2,180*2,n_bands])

  ;; 0.5x0.5 deg Refrectance and Albedo group (data_type 4 = 32bit)
  map_sp_ref = read_binary(ifname_ref, data_type=4, data_dims=[360*2,180*2,n_bands])
